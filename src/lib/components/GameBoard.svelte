<script>
  ...
  $: {
    ...
    if (level === 'adulte') {
      ...
      // Quand toutes les cases sont résolues, la grille précédente est
      // comptabilisée dans le total des cellules résolues
      if (count === 0 && totalSolvedCountAdult > 0) {
        // Ne rien faire, car la grille vient d'être réinitialisée
      } else {
        solvedCountAdult = count;
      }
    }
  }
  ...
  // Mise à jour du compte total des cellules résolues
  $: {
    if (level === 'adulte') {
      // Si solvedCountAdult diminue significativement alors qu'il était élevé, c'est probablement une réinitialisation
      if (solvedCountAdult < 10 && totalSolvedCountAdult >= 90) {
        // La grille a été réinitialisée, ajouter 100 au total
        totalSolvedCountAdult += 100;
      } else if (solvedCountAdult > totalSolvedCountAdult % 100) {
        // Mise à jour régulière
        totalSolvedCountAdult = Math.floor(totalSolvedCountAdult / 100) * 100 + solvedCountAdult;
      }
    } else if (level === 'enfant') {
      // Même logique pour le mode enfant
      const total = solvedCountChild.total;
      if (solvedCountChild.count < 10 && totalSolvedCountChild.count >= (total * 0.9)) {
        // La grille a été réinitialisée, ajouter le total précédent
        totalSolvedCountChild.count += total;
      } else if (solvedCountChild.count > totalSolvedCountChild.count % total) {
        // Mise à jour régulière
        totalSolvedCountChild.count = Math.floor(totalSolvedCountChild.count / total) * total + solvedCountChild.count;
      }
