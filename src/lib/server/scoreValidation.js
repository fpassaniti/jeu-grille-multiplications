 ...
 * @param {any} body
 * @param {number} extraSec Secondes supplémentaires accordées par une potion
 *   de bonus de temps déjà vérifiée côté serveur (jamais dérivée du payload
 *   client) — élargit le plafond nominal `duration*60` en conséquence.
 * @returns {{error: string}|{value: Object}}
 */
export function normalizePayload(body, extraSec = 0) {
  const score = Number(body.score);
  const duration = parseInt(body.duration, 10);

  if (body.score == undefined || !body.duration) {
  ...
  // borné à [1, duration*60 + extraSec] — un ancien client sans elapsedSec
  // suppose la durée nominale. `extraSec` vient d'une potion de bonus de
  // temps déjà vérifiée côté appelant (cf. verifyAndConsumePotions), jamais
  // du payload client.
  const maxSec = duration * 60 + extraSec;
  const elapsedSec = Math.min(
    maxSec,
    Number.isInteger(body.elapsedSec) && body.elapsedSec > 0 ? body.elapsedSec : maxSec
  );
