-- 007_reward_completion_gate.sql — corrige deux failles de farming de pièces (§5.2) :
-- 1. les bonus forfaitaires (première partie, streak, week-end, booster, perfect)
--    ne sont plus accordés que si la partie est allée à son terme naturel
--    (minuteur écoulé), jamais via le bouton « Terminer la partie » ;
-- 2. le bonus de streak n'est plus recrédité qu'une fois par jour (comme le
--    bonus « première partie du jour »), au lieu de chaque partie complétée.
-- Migration additive et idempotente (CREATE OR REPLACE). Le score de base
-- (v_base) et l'XP/games_played/total_score restent crédités dans tous les
-- cas — seuls les bonus non proportionnels à l'effort réel sont concernés.
-- Les parties à 0 point sont filtrées en amont, côté API (/api/scores),
-- avant même d'appeler cette fonction.

-- Postgres distingue les fonctions par signature : ajouter un paramètre crée
-- une surcharge à 4 arguments à côté de l'ancienne à 3, elle ne la remplace
-- pas. On supprime explicitement l'ancienne pour qu'aucun appel resté sur 3
-- arguments ne retombe silencieusement sur le comportement non corrigé.
DROP FUNCTION IF EXISTS add_game_rewards(UUID, INTEGER, BOOLEAN);
