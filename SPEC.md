...
- **Division** (rappel) : un seul fait mémorisé (pas un algorithme multi-étapes) → la grille de difficulté psychologique existante est conservée pour sa forme relative mais **rescalée** dans une plage resserrée `RECALL_DIFFICULTY_RANGE = [0.3, 0.7]` / `RECALL_TIME_RANGE_SEC = [6, 10] s` (adulte) via `rescaleRecall()`, pour ne pas rivaliser avec les modes posés.
- **Tables** : **revert (2026-08-11)** à la formule de score V1 exacte (`computeLegacyWholeScore`, `scoring.js`) — `Math.round(timeRemaining × difficultyMultiplier)` (adulte) / `Math.round(timeRemaining × (difficultyMultiplier × 0.7 + 0.3))` (enfant), avec la grille de difficulté brute `DIFFICULTY_MATRIX` (0.5–3.0, pic 7×7) et le temps brut `5–15 s` (adulte), **sans** le rescale `RECALL_DIFFICULTY_RANGE`/`RECALL_TIME_RANGE_SEC` appliqué entre le 23/07/2026 et le 11/08/2026. Motif : ce rescale avait réduit le score des tables difficiles jusqu'à ~4×, rendant le classement (`/api/leaderboard`, filtré par mode/niveau/durée) incomparable aux scores historiques enregistrés sous l'ancienne formule. Les tables ne sont donc plus alignées sur le modèle « opérations élémentaires » — c'est un choix délibéré, pas un oubli. La question générée porte `legacyWhole: true` pour que `GameEngine#markQuestionSolved` utilise ce chemin au lieu de `computeScore`.
- Ce revert a fait retomber le plancher ×6 (M6 vs table 7×7, voir §4.5) à ×3.25 — `POSED_SCORE_CALIBRATION` est passée de `1` à `2` pour le restaurer (×6.5 avec marge), plutôt que de recompresser les tables (décision produit explicite : en cas de déséquilibre, on réhausse les modes posés). C'est un ajustement déterministe (plancher théorique par question), pas une calibration de score de session réel — voir le commentaire de `balance-config.js` pour la méthode à suivre si un déséquilibre est constaté en jeu réel.
- **Décote anti-abus des sélections de tables faciles (2026-08-16)** : une analyse des `game_sessions` en base (Neon) a montré un farming réel du mode enfant restreint aux tables {1,2,10} (jusqu'à 2,5× plus de bonnes réponses/minute que le reste de la grille, un compte ayant enchaîné 23 sessions en une journée). `tables.js#createGenerator` calcule désormais un `coverageFactor` = moyenne de `tableDifficulty` sur le pool effectivement jouable (déterminé par `selectedTables`) divisée par la moyenne de la grille complète (`fullGridAverageDifficulty()`, dérivée dynamiquement de `DIFFICULTY_MATRIX`), plafonné à `[MIN_COVERAGE_FACTOR (0.4), 1]`. Ce facteur multiplie la `difficulty` portée par chaque question générée — **pas** `tableDifficulty()` elle-même, qui reste la valeur brute testée par `balance.test.js`/`tables.test.js`. Hors mode enfant restreint (adulte, ou enfant avec les 10 tables cochées), le pool = grille complète et `coverageFactor` vaut naturellement 1 : aucun impact sur le score « normal ». C'est une décote de session ajoutée par-dessus la formule V1 (qui reste inchangée cellule par cellule) — les scores déjà enregistrés en base ne sont pas concernés, seules les nouvelles parties avec un pool restreint le sont.
- `MAX_DIFFICULTY = 8.0` (M6) → `maxPointsPerQuestion() = 253` (utilisé par l'anti-triche §5.7, dérivé dynamiquement dans `scoreValidation.js` pour éviter le drift ; inclut désormais aussi le plafond théorique de la formule legacy des tables).
- Le facteur enfant `LEVEL_TIME_FACTOR = ×3` (`generator-utils.js`) reste global ; pour les tables (formule V1), le temps de base va jusqu'à 15 s adulte / 45 s enfant (au lieu du `[6, 10] s` du modèle rappel, toujours en vigueur pour division).
- Testé par `src/lib/game/balance.test.js` (invariant : `difficulty` proportionnelle à `operationCount` pour les modes posés, bornes de temps par niveau, plancher ×6 M6/table).

### 4.5 Paliers pédagogiques (échelle de difficulté commune)

Difficulté et temps alloué (adulte) dérivés du modèle par opérations élémentaires (§4.4, `balance-config.js`) : `operationCount` = nb d'opérations à un chiffre de l'algorithme posé (+1 si retenue/emprunt) pour les modes posés ; rappel resserré `[0.3, 0.7]`/`[6, 10] s` pour division uniquement. Les tables utilisent leur propre formule V1 (§4.4) : difficulté brute `[0.5, 3.0]`, temps `[6, 15] s` adulte.
...
| Tables | — | Matrice 10×10 existante (non posé, formule V1 exacte — pas le modèle rappel ci-dessus) | — | 0.5–3.0 (brut) | 6–15 s |

M6 (score max théorique, chiffre par chiffre) / table 7×7 (score max théorique, formule V1) ≈ 234/36 ≈ **×6,5** — au-delà du plancher ×6 demandé, grâce à `POSED_SCORE_CALIBRATION = 2` (§4.4). Enfant = ×3 sur le temps adulte (`LEVEL_TIME_FACTOR`, `generator-utils.js` pour les modes posés/division ; formule dédiée dans `tableTime()` pour les tables) : M6 enfant = 135 s (2 min 15), tables enfant ≤ 45 s.
...
- `QuestionPanel.svelte` : `isPosed = question.posed`. Pour chaque case, position depuis la droite `p = stage.digits - 1 - j` (j = index visuel gauche→droite) ; verrouillée (verte, définitive) si `p < digitIndex` ou ligne antérieure ; active (curseur clignotant, rouge pendant le flash d'erreur) si `p === digitIndex` de la ligne courante ; sinon en attente (grisée). Le décalage des produits partiels (`shift`) reste purement visuel (cases « spacer » invisibles ajoutées à droite), l'enfant ne tape aucun zéro de décalage. Un seul `<input>` réel (visuellement masqué, focus repris au clic n'importe où sur la zone posée) reste la source de saisie unique — clavier physique ou clavier numérique natif du navigateur/OS (`inputmode="numeric"`, aucun pavé custom) — les cases ne sont qu'un reflet d'affichage : pas de gestion de focus multi-input à maintenir. Sur `keydown`, `digitFromKeydown` lit `event.code` (position physique de la touche, ex. `Digit5`) plutôt que `event.key` (caractère produit) pour taper directement le bon chiffre quelle que soit la disposition clavier (AZERTY, etc.) sans devoir maintenir Shift ; le pavé numérique (`NumpadN`) et le clavier virtuel tactile (pas de `code` exploitable) ne matchent jamais ce motif et suivent donc le flux `on:input` natif inchangé (2026-08-12).
